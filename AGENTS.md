...
正式前端任务必须同时接收批准战略和后端真实接口交付；按 `.template-spec/process/frontend-backend-delivery.md` 在启动、恢复及阶段边界重验。仅 Harness Entry 的无写权限 Explorer 可先诊断缺口。输入通过后可准备工程计划和合同，代码仍需合同已批准且当前及既有 ready-for-agent 条件。
