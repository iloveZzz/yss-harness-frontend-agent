...
触发面很窄。全新产品、全新模块和低风险调整不生成 Spec Delta。`docs/process/harness-process-tailoring.md` 把「已有冻结基线的高风险变更」列为必需工作：Spec Delta、战术设计 / 数据 / 工程审查、契约复核、切片和回滚设计。未命中时按 [[条件强制门禁]] 与 [[影响面分诊与流程裁剪]] 记录 `not-applicable` 及原因，不写空 Delta。明确改变认证或授权行为时，把差异当作普通行为写入本 Delta，不另开专项 Delta。
...
- kind: Disputed
- sources: docs/templates/spec-delta-template.md, AGENTS.md, docs/process/lifecycle-registry.yaml
