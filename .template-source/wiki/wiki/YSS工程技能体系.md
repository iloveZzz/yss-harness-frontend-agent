...
词汇上的分层是：核心技能默认可发现，负责生命周期控制或通用研发入口；专项技能由 实现合同编译器 按影响面和实现合同按需选择；试验技能只在明确试验范围内使用。`.template-spec/agents/yss-skill-registry.yaml` 记录分层、别名、默认可发现性和运行时入口，当前 `status: active`，实现合同编译器 与生命周期必须消费通过本表校验的 canonical 技能。
...
创建、修改或退役 skill 时使用 `maintaining-skills`，并按 [[模板维护流程]] 判定 L1 / L2 / L3。已退役入口见 `.template-spec/agents/skill-migrations.md` 与 [[技能投影与锁定]]。本地持久知识库走 [[LLM Wiki]]。
