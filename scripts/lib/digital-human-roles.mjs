...
function validateRuntimes(doc, skillRegistry, groupSizes) {
  ...
  for (const runtime of doc.runtimes) {
    ...
    if (runtime.id === "runtime.grok") {
      ...
      if (runtime.max_collaboration_group_size !== 4) fail("runtime.grok 群聊上限必须为 4");
...
export function validateDigitalHumanRoles(doc, { skillIds, stageIds, gateIds, artifactIds, evidenceIds, workUnitIds, skillRegistry } = {}) {
  ...
  if (runtime.lifecycle_control !== "harness-orchestrator") fail("runtime_policy.lifecycle_control 必须为 harness-orchestrator");
  ...
  if (!Array.isArray(orchestrator.default_dual_hat) || orchestrator.default_dual_hat.length !== 0) fail("orchestrator.default_dual_hat 必须为空数组");
