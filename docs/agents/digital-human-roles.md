...
| 角色 | 主要职责 | 明确不负责 |
|---|---|---|
| `role.architecture-agent` | 验证 Strategic Preflight，把已批准战略输入、视觉基线和条件化 Backend Delivery 细化为前端工程设计 | 本地 Tactical DDD、后端架构分流、生产实现、独立验证 |
| `role.frontend-agent` | 消费 Slice Contract，交付页面、组件、状态、API 消费和前端测试 | 创造产品规则、修改后端领域 / API 契约、验收自己实现的切片 |
| `role.backend-agent` | 仅审查上游 Backend Delivery 身份和 API / 数据边界，必要时反馈不匹配 | 在前端模板内执行后端设计或实现、验收自己实现的切片 |
...
`harness-entry → frontend-engineering-design → slice-contract → slice-implementation → verification`
...
`work-unit.frontend-engineering-design` 由架构 Agent 起草、测试 Agent 会签；`gate.slice-contract-approved` 由架构 Agent 起草并由前端、后端、测试 Agent 会签；`gate.fresh-verification-passed` 由测试 Agent 起草、架构 Agent 会签。高风险架构、OpenAPI Freeze 和 `gate.merge-approved` 保留生物人会签。运行时副作用审批始终由生物人负责。
