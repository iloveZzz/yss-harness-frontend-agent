...
| 后端脚手架策略 | `orchestration-contract.yaml` / `scaffold-architecture-decisions.yaml` |  | backend `scaffold_status=required` 时记录 DDD/MVC 推荐、逐项目用户确认、批准状态、schema v3 合同、实际生成器、预期文件、`./mvnw` 结果和后置合同重编译 |
...
| 后端脚手架顺序：工程基线 → Agent 推荐与用户逐项目确认 → Harness Orchestrator 批准架构记录 → 实现合同编译器 schema v3 draft / 批准 → 对应 DDD/MVC 生成器 → 合同重编译；DDD 追加父基线校验；脚手架不得生成业务行为 | Harness Orchestrator / DDD 或 MVC 生成器 |  | `implemented` / `seam-deferred` / `drift` / `violation` / `not-applicable` |  | 未确认、选择 digest 漂移、生成器不匹配或缺 Execution Result 时阻断 |
